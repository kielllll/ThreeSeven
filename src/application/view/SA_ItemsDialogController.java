...
public class SA_ItemsDialogController implements Initializable {
	@FXML
	...
	private static SA_ItemsDialogController instance = new SA_ItemsDialogController();
		...
		try {
			...
			btnSave.setOnAction(e->{
				if(hasIncompleteFields())
					...
					if((StockAdjustmentController.getInstance().isItemExisting(p.getName()))) {
						StockAdjustmentController.getInstance().update(p.getItemID(), qty);
					} else {
						StockAdjustmentController.getInstance().add(p.getItemID(), Integer.parseInt(txtQuantity.getText()));
					}
				}
			});
			...
			btnCancel.setOnAction(e->{
				Dialogs.getInstance().close("SA_Item");
			});
		} catch(Exception err) {
			err.printStackTrace();
		}
	...
	public static SA_ItemsDialogController getInstance() {
		return instance;
	}
}
