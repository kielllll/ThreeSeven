...
public class SO_ItemsDialogController implements Initializable {
	private static SO_ItemsDialogController instance = new SO_ItemsDialogController();
		...
		try {
			...
			btnSave.setOnAction(e->{
				if(hasIncompleteFields())
					...
					if((StockOutController.getInstance().getItemQty(p.getName())+qty)>p.getQuantity()||qty>p.getQuantity())
						Components.showError(lblError, "Error: Quantity requested is greater than current quantity");
					else {
						if(StockOutController.getInstance().isItemExisting(p.getName())) {
							StockOutController.getInstance().update(p.getItemID(), Integer.parseInt(txtQuantity.getText()));
						} else {
							StockOutController.getInstance().add(p.getItemID(), Integer.parseInt(txtQuantity.getText()));
						}	
					}
				}
			});
			...
			btnCancel.setOnAction(e->{
				Dialogs.getInstance().close("SO_Item");
			});
		} catch(Exception err) {
			err.printStackTrace();
		}
	...
	public static SO_ItemsDialogController getInstance() {
		return instance;
	}
}
