...
public class PO_ItemsDialogController implements Initializable {
	@FXML
	...
	private static PO_ItemsDialogController instance = new PO_ItemsDialogController();
		...
		try {
			...
			btnSave.setOnAction(e->{
				if(hasIncompleteFields())
					...
					if(PurchaseOrderController.getInstance().isItemExisting(p.getName())) {
						PurchaseOrderController.getInstance().update(p.getItemID(), Integer.parseInt(txtQuantity.getText()));
						PurchaseOrderController.getInstance().updateCost();
					} else {
						PurchaseOrderController.getInstance().add(p.getItemID(), Integer.parseInt(txtQuantity.getText()));
						PurchaseOrderController.getInstance().updateCost();
						PurchaseOrderController.getInstance().disableCBSupplier(true);
					}
				}
			});
			...
			btnCancel.setOnAction(e->{
				Dialogs.getInstance().close("PO_Item");
			});
		} catch(Exception err) {
			err.printStackTrace();
		}
	...
	public void initList() {
		try {
			...
			List<Item> tempList = ItemDAOImpl.getInstance()
											.getAll()
											.parallelStream()
											.filter(i->i.getSupplier().equals(PurchaseOrderController.getInstance().getSupplier()))
	...
	public static PO_ItemsDialogController getInstance() {
		return instance;
	}
}
